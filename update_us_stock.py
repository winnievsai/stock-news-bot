...
FinMind 美股日K 增量抓取工具
================================
用途：讀取 us_stock_list.txt 中的股票代碼，透過 FinMind API 抓取「美股日K」資料
     (dataset = USStockPrice)，每支股票各自存成一個 CSV。
...
注意事項：
    - 美股代碼要用大寫（FinMind 對小寫代碼一律回傳 0 筆，不會報錯，容易誤判成沒資料）
    - 少數代碼（例如 BRK.B）FinMind 目前查不到股價，程式會顯示「無新資料」並繼續下一檔，
      不會中斷整個流程

設定（.env.local，跟其他腳本共用同一份）：
    FINMIND_TOKEN=你的token                    (必填，與台股抓取共用同一組token/額度)
    US_BACKFILL_START_DATE=2024-01-01          (選填，預設 2024-01-01；只在該股票第一次
                                                 建立CSV時使用)
    US_OUTPUT_DIR=us_data                      (選填，預設 ./us_data)
    US_STOCK_LIST_FILE=us_stock_list.txt       (選填，預設 ./us_stock_list.txt)
    REQUEST_INTERVAL_SEC=6.5                    (選填，預設 6.5 秒，與其他腳本共用同一個
                                                 FinMind 額度上限)
...
def load_config():
    ...
    backfill_start = env.get("US_BACKFILL_START_DATE", DEFAULT_BACKFILL_START).strip()
    output_dir = SCRIPT_DIR / env.get("US_OUTPUT_DIR", "us_data").strip()
...
def main():
    ...
    print(f"共 {len(stocks)} 檔美股，輸出資料夾：{output_dir}")
